	      ...
	      char *mdid)
{
	...
	fprintf(stderr,
		...
		authdata ? authdata : "<null>", anc, mdid);
	       ...
	       int timeout)
{
	...
	if (debug)
	    fprintf(stderr, "xdmcp_dispatch: op=ACCEPT, session=%lu, auname=%s, audata=%s, azname=%s, azdata=%s\n",
		    session, auname, data_to_str(audata, audlen), azname, data_to_str(azdata, azdlen));
	...
	if (debug)
	    fprintf(stderr, "xdmcp_dispatch: op=FAILED, session=%lu, status=%s (saved session=%lu)\n",
		    sid, status, session);
	
	if (state == XDMCP_STATE_AWAIT_MANAGE_RESPONSE ||
	    state == XDMCP_STATE_AWAIT_ALIVE)
	{
	    if (session == sid)
	    {
		status = array8_to_str(&cp);
     ...
     char *argv[])
{
    ...
    for (i = 1; i < argc && argv[i][0] == '-'; ++i)
		...
		break;
	      ...
	      case 'h':
		printf("Usage: %s [-h] [-V] [-d[<level>]] [-v[<level>]] [-D<display>] [-a<authfile>] [-A<xauthprog>]\n",
		       argv[0]);
		exit(0);
	...
	*cp++ = '\0';
	...
	switch (state)
	{
	  ...
	  case XDMCP_STATE_START_CONNECTION:
	    ...
	    rc = xdmcp_request(fd, dpy, 1, ctv, 1, cal, cav, authname, authdata, 1, anv, "xdmcp client");
	    if (rc < 0)
		goto End;
